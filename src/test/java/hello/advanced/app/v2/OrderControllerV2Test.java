...
class OrderControllerV2Test {
    @LocalServerPort
    private int port;
    private final String URI = "/v2/request";
    ...
    @Test
    void exceptionTest() {
        ExtractableResponse<Response> 예외_주문_요청 = RestAssured.given().log().all()
            .queryParam("itemId", "ex")
            .when().get(URI)
            .then().log().all().extract();

        Assertions.assertThat(예외_주문_요청.statusCode()).isEqualTo(HttpStatus.INTERNAL_SERVER_ERROR.value());
    }
}
